...
if __name__=='__main__':
    ...
    print(f'  無差別基準線(全C(n,3)組合せ購入・必ず的中): {base:.2f}%')
    # ⚠ 旧版の注記2行は第12報§8・第17報§8で撤回済み。以下が撤回後の正しい意味づけ:
    #   (a) 払戻率75%は「投票比率＝真の確率」のとき全通り買いが返す値であって、
    #       「等確率のときだけ到達する上限」ではない。
    #   (b) 75%を下回ることは「買われ過ぎ」の証拠にならない。全通り買いの回収率は
    #       (1-控除率)×mean(真の確率/投票比率) という比の非加重平均であり、
    #       Σ投票比率>Σ真の確率とは別命題。単勝の全頭等額買いは定義上Σが一致するが
    #       実測70.96%で名目80%を9.04pt下回る(同一16,660R)。
    print(f'  ※JRA3連複払戻率75%は「投票比率＝真の確率」のときの値。上限でも買われ過ぎの基準でもない\n')
