#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""複合（係またぎ）9/6 v1（9/5 v3 と同じ定義）: ⑤MB/UL × ⑭次走(v1.4) × 穴帯(基準人気7〜12) × ⑥調教 × ⑨運勢 を1頭ごとに並べる。
...
TK = json.load(open(os.path.join(D, 'toukei_20260906.json'))); T = TK['races']
C = json.load(open(os.path.join(D, 'chokyo_20260906.json')))['horses']
U = json.load(open(os.path.join(D, 'unsei_20260906.json')))['horses']
J = {(h['ba'], h['r'], h['uma']): h for h in json.load(open(os.path.join(D, 'jisou906.json')))['hits']}
rows = []
for rc in T:
    for h in rc['horses']:
        if h['scratched']:
        ...
        rows.append(dict(ba=rc['ba'], r=rc['r'], title=rc['title'], td=rc['td'], dist=rc['dist'],
                         pattern=rc.get('pattern'), c1=rc.get('c1'), t6=rc.get('t6'), ryoritsu=rc.get('ryoritsu'),
                         ...
                         MB=h['MB'], UL=h['UL'], UL_REF=h.get('UL_REF', []),   # UL_REF=正本外(退避テンプレ)。flagsに数えない=[要確認]
                         chokyo=cho, chokyo_status=c.get('status'),
...
json.dump(dict(version='composite906_v1', raceday='20260906', def_version=dict(TK.get('def_version', {}), **{
    '複合フラグ': 'MB/UL(正本のみ)/次走(jisou v1.4)/穴帯(kijun_ninki 7-12)/調教(best_c2∈{A3,A2} or last_d2.z1f≤-1.0)/運勢(jockey_mark_usable∈{◎◎,◎}・素材一致のみ)。v3 2026-09-05〜',
    'nflag': 'flagsの本数。序列・軸ではない（9/5第9報: 3本以上は母集団並み）'}), rows=rows),
          open(os.path.join(D, 'composite.json'), 'w'), ensure_ascii=False, indent=1)
