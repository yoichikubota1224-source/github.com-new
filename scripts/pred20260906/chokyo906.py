#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""⑥調教係(2026-09-06): 8/30〜9/5の追切を9/6出走484頭へ結合する。9/5 chokyo905 v2 と同じ手順。
...
未来情報混入の防止: 採用する追切日は 20260830〜20260905（すべて 20260906 より前）。
"""
...
# ---------------------------------------------------------------- 出走表
races = json.load(open(os.path.join(REPO, 'predictions', RACEDAY, f'toukei_{RACEDAY}.json')))['races']
entries = []
for rc in races:
    for h in rc['horses']:
...
for e in entries:
    ...
    if not got:
        ...
        out[key] = dict(name=e['name'], hid=e['hid'], scratched=e['scratched'],
                        status='[不足]', reason='8/30〜9/5に追切の記録が無い',
                        n_works=0, join=None)
        continue
    ...
    d2 = [x for x in got if x['date'] <= '20260904']
