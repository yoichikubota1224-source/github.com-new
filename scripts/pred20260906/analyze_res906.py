...
res   = json.load(open(P('predictions/20260906/results_20260906.json'), encoding='utf-8'))
comp  = json.load(open(P('predictions/20260906/composite.json'), encoding='utf-8'))
comp  = comp['rows'] if isinstance(comp, dict) else comp
touk  = json.load(open(P('predictions/20260906/toukei_20260906.json'), encoding='utf-8'))
touk  = touk['races'] if isinstance(touk, dict) else touk
unsei = json.load(open(P('predictions/20260906/unsei_20260906.json'), encoding='utf-8'))
unsei = unsei.get('horses', unsei) if isinstance(unsei, dict) else unsei
chok  = json.load(open(P('predictions/20260906/chokyo_20260906.json'), encoding='utf-8'))
jis   = json.load(open(P('predictions/20260906/jisou906.json'), encoding='utf-8'))
myo   = json.load(open(P('predictions/20260906/myoumi_20260906.json'), encoding='utf-8'))
myo   = myo.get('races_all', myo.get('races', myo)) if isinstance(myo, dict) else myo
...
for lab, sel in [('0-1', lambda v: v <= 1), ('2-3', lambda v: 2 <= v <= 3),
    ...
    hs = [R[(rc['ba'], rc['r'], h['uma'])] for rc in myo for h in rc['rows']
