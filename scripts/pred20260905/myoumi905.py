#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-09-05 オッズ妙味分析（既報7R: 中山7・8・11R／札幌8・9・10R／阪神11R）。8/30の手法（ana_20260830.py）を踏襲。
...
CM = {(x['ba'], x['r'], x['uma']): x for x in json.load(open(os.path.join(D, 'composite.json')))}
J = {(x['ba'], x['r'], x['uma']): x for x in json.load(open(os.path.join(D, 'jisou905.json')))['hits']}
...
R7 = [('中山', 7), ('中山', 8), ('中山', 11), ('札幌', 8), ('札幌', 9), ('札幌', 10), ('阪神', 11)]
...
sel = [o for o in out if (o['ba'], o['r']) in R7]
res = dict(version='myoumi905_v1', raceday='20260905', method=__doc__, diag=diag, races=sel)
json.dump(res, open(os.path.join(D, 'myoumi_7R_20260905.json'), 'w'), ensure_ascii=False, indent=1)
# CSV（7R全頭）
...
with open(os.path.join(D, '08_オッズ妙味_7R_20260905.csv'), 'w', encoding='utf-8-sig', newline='') as f:
    w = csv.writer(f, lineterminator='\n'); w.writerow(['target_date'] + cols + ['v21', 'decision'])
    for o in sel:
        for h in o['rows']:
            w.writerow(['2026-09-05'] + [(' / '.join(x) if isinstance(x, list) else ('・'.join(x) if False else x)) if k != 'support' and k != 'hist' and k != 'flags' else (' / '.join(x) if x else '') for k, x in ((k, (o[k] if k in ('ba', 'r') else h.get(k))) for k in cols)] + ['HOLD', '買い目・印・軸なし'])
print('diag', diag)
for o in sel:
